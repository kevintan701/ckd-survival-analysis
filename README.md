...
> **Note:** `data/raw/` and `data/processed/` are excluded from the repo by `.gitignore` — they are publicly available from CDC and regenerated by running the pipeline. All output figures (`output/figures/`) are committed and needed for the report.
